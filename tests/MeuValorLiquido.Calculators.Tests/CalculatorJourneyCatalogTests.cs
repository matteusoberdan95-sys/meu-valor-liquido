...
public sealed class CalculatorJourneyCatalogTests
{
    ...
    public void Saida_Empresa_Should_Expose_Fgts_And_Faq_From_Rescisao()
    {
        ...
        steps[1].Target.Should().Be("seguro-desemprego");
        steps[1].Kind.Should().Be(CalculatorJourneyStepKind.Calculator);
        steps[2].Kind.Should().Be(CalculatorJourneyStepKind.Faq);
        steps[2].Target.Should().Be("multa-fgts-40-porcento");
    }
