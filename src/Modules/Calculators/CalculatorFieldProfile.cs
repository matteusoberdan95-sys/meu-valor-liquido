namespace MeuValorLiquido.Modules.Calculators;
public sealed record CalculatorFieldProfile(
    bool ShowAmount = true,
    bool ShowSecondaryAmount = false,
    bool ShowMonths = false,
    bool ShowRate = false,
    bool ShowHours = false,
    bool ShowDependents = false,
    bool ShowTransportDiscount = false,
    bool ShowMealVoucherDiscount = false,
    bool ShowHealthPlanDiscount = false,
    bool ShowAlimonyAmount = false,
    bool ShowAlimonyPercent = false,
    bool ShowOtherDiscounts = false,
    bool ShowTerminationReason = false,
    bool ShowNoticePeriod = false,
    bool ShowHasUnpaidVacation = false,
    bool ShowVacationTakenInPeriod = false,
    bool ShowMonthsSinceLastVacation = false,
    bool ShowCompleteYears = false,
    bool ShowFgtsBalance = false,
    bool ShowThirteenthMonths = false,
    bool ShowTerminationMonth = false,
    bool ShowAdmissionMonth = false,
    bool ShowAdmissionInPriorYear = false,
    bool ShowAdmissionDate = false,
    bool ShowTerminationDate = false,
    bool ShowNoticePeriodOptions = false,
    bool ShowWeeklyWorkHours = false,
    bool ShowOvertimeShiftType = false,
    bool SecondaryAmountIsDays = false,
    bool AmountIsCurrency = true,
    string AmountLabel = "Valor principal",
    string SecondaryAmountLabel = "Valor secundário",
    string MonthsLabel = "Meses",
    string RateLabel = "Taxa (%)",
    string HoursLabel = "Horas",
    string DependentsLabel = "Dependentes",
    string TransportDiscountLabel = "Descontos extras",
    string MealVoucherDiscountLabel = "Vale-refeição/alimentação",
    string HealthPlanDiscountLabel = "Plano de saúde",
    string AlimonyAmountLabel = "Pensão alimentícia (valor)",
    string AlimonyPercentLabel = "Pensão alimentícia (%)",
    string OtherDiscountsLabel = "Outros descontos",
    string TerminationReasonLabel = "Tipo de desligamento",
    string NoticePeriodLabel = "Cumpriu aviso prévio de 30 dias?",
    string HasUnpaidVacationLabel = "Possui férias vencidas (não gozadas)?",
    string VacationTakenInPeriodLabel = "Já gozou férias neste período aquisitivo?",
    string MonthsSinceLastVacationLabel = "Meses desde a última férias (proporcional parcial)",
    string CompleteYearsLabel = "Anos completos na empresa",
    string FgtsBalanceLabel = "Saldo FGTS (opcional — para multa)",
    string ThirteenthMonthsLabel = "Meses trabalhados no ano da saída (13º)",
    string TerminationMonthLabel = "Mês da rescisão",
    string AdmissionMonthLabel = "Mês de admissão",
    string AdmissionInPriorYearLabel = "Admissão no ano anterior à rescisão",
    string AdmissionDateLabel = "Data de admissão",
    string TerminationDateLabel = "Data de afastamento",
    string NoticePeriodOptionLabel = "Aviso prévio",
    string WeeklyWorkHoursLabel = "Jornada semanal (horas)",
    string OvertimeShiftTypeLabel = "Tipo de hora extra",
    bool ShowSalaryBasis = false,
    bool ShowMeiActivity = false,
    bool ShowFinancingAmortization = false,
    bool ShowFgtsTerminationReason = false,
    bool ShowLayeredOptionalFields = false,
    bool ShowIrrfGrossSalaryToggle = false,
    bool ShowVacationOptions = false,
    bool ShowThirteenthAdvance = false,
    bool ShowSalaryAverageSupplement = false,
    bool ShowSimplesAnnex = false,
    bool ShowProLaborePercent = false,
    bool ShowMeiAnnualAccumulated = false,
    string LayeredOptionalSectionLabel = "Ajustar descontos e dependentes",
    string IrrfGrossSalaryToggleLabel = "Informar salário bruto (descontar INSS automaticamente)",
    string VacationDayOptionLabel = "Dias de férias",
    string SellVacationAllowanceLabel = "Vender até 1/3 dos dias (abono pecuniário)",
    string DoubleVacationPaymentLabel = "Férias em dobro (atraso na concessão)",
    string ThirteenthAdvancePaidLabel = "Adiantamento do 13º já recebido",
    string SalaryAverageSupplementLabel = "Média mensal de HE/comissão (opcional)",
    string SimplesAnnexLabel = "Anexo do Simples Nacional",
    string ProLaborePercentLabel = "Pró-labore (% do faturamento)",
    string MeiAnnualAccumulatedLabel = "Faturamento já acumulado no ano (opcional)",
    string SalaryBasisLabel = "Tipo de valor informado",
    string MeiActivityLabel = "Atividade MEI",
    string FinancingAmortizationLabel = "Sistema de amortização",
    string FgtsTerminationReasonLabel = "Tipo de desligamento (para multa)");

public interface ICalculatorFieldProfileProvider
{
    CalculatorFieldProfile GetProfile(string slug);
}

public sealed class CalculatorFieldProfileProvider : ICalculatorFieldProfileProvider
{
    private static readonly Dictionary<string, CalculatorFieldProfile> Profiles = new(StringComparer.OrdinalIgnoreCase)
    {
        ["salario-liquido"] = new(
            AmountLabel: "Salário bruto",
            ShowDependents: true,
            ShowTransportDiscount: true,
            TransportDiscountLabel: "Vale-transporte",
            ShowMealVoucherDiscount: true,
            ShowHealthPlanDiscount: true,
            ShowAlimonyAmount: true,
            ShowAlimonyPercent: true,
            ShowOtherDiscounts: true,
            OtherDiscountsLabel: "Outros descontos (empréstimo, sindicato etc.)",
            ShowLayeredOptionalFields: true,
            LayeredOptionalSectionLabel: "Ajustar descontos"),
        ["salario-bruto-necessario"] = new(
            AmountLabel: "Salário líquido desejado",
            ShowDependents: true,
            ShowTransportDiscount: true,
            TransportDiscountLabel: "Vale-transporte",
            ShowMealVoucherDiscount: true,
            ShowHealthPlanDiscount: true,
            ShowAlimonyAmount: true,
            ShowAlimonyPercent: true,
            ShowOtherDiscounts: true,
            OtherDiscountsLabel: "Outros descontos",
            ShowLayeredOptionalFields: true,
            LayeredOptionalSectionLabel: "Ajustar descontos"),
        ["proposta-salarial"] = new(
            AmountLabel: "Salário bruto atual",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Salário bruto proposto",
            ShowDependents: true,
            ShowTransportDiscount: true,
            TransportDiscountLabel: "Vale-transporte",
            ShowMealVoucherDiscount: true,
            ShowHealthPlanDiscount: true,
            ShowAlimonyAmount: true,
            ShowAlimonyPercent: true,
            ShowOtherDiscounts: true,
            OtherDiscountsLabel: "Outros descontos mensais",
            ShowLayeredOptionalFields: true,
            LayeredOptionalSectionLabel: "Ajustar descontos"),
        ["ferias"] = new(
            AmountLabel: "Salário base",
            ShowDependents: true,
            ShowMonths: true,
            MonthsLabel: "Meses de férias (12 = integral)",
            ShowAdmissionDate: true,
            ShowTerminationDate: true,
            AdmissionDateLabel: "Data de admissão (opcional)",
            TerminationDateLabel: "Data de afastamento (opcional)",
            ShowLayeredOptionalFields: true,
            ShowVacationOptions: true,
            LayeredOptionalSectionLabel: "Ajustar opções e dependentes"),
        ["decimo-terceiro"] = new(
            AmountLabel: "Salário base",
            ShowMonths: true,
            MonthsLabel: "Meses trabalhados no ano",
            ShowAdmissionDate: true,
            ShowTerminationDate: true,
            AdmissionDateLabel: "Data de admissão (opcional)",
            TerminationDateLabel: "Data de referência / saída",
            ShowDependents: true,
            ShowLayeredOptionalFields: true,
            ShowThirteenthAdvance: true,
            LayeredOptionalSectionLabel: "Ajustar datas, dependentes e adiantamento"),
        ["rescisao-clt"] = new(
            AmountLabel: "Último salário (bruto)",
            ShowAdmissionDate: true,
            ShowTerminationDate: true,
            ShowMonths: true,
            MonthsLabel: "Tempo total na empresa (meses) — preenchido pelas datas",
            ShowAdmissionMonth: true,
            ShowAdmissionInPriorYear: true,
            ShowThirteenthMonths: true,
            ShowTerminationMonth: true,
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Dias trabalhados no mês da saída",
            SecondaryAmountIsDays: true,
            ShowTerminationReason: true,
            TerminationReasonLabel: "Motivo do desligamento",
            ShowNoticePeriod: true,
            ShowNoticePeriodOptions: true,
            NoticePeriodLabel: "Aviso prévio (legado)",
            ShowDependents: true,
            ShowHasUnpaidVacation: true,
            ShowVacationTakenInPeriod: true,
            ShowMonthsSinceLastVacation: true,
            ShowCompleteYears: true,
            ShowFgtsBalance: true,
            ShowTransportDiscount: true,
            TransportDiscountLabel: "Descontos do último mês (VT, empréstimo etc.)",
            ShowThirteenthAdvance: true,
            ShowSalaryAverageSupplement: true),
        ["hora-extra"] = new(
            AmountLabel: "Valor da hora",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Salário mensal bruto (opcional)",
            ShowHours: true,
            ShowRate: true,
            RateLabel: "Adicional CCT (%) — ex.: 50, 70, 100",
            ShowWeeklyWorkHours: true,
            ShowOvertimeShiftType: true),
        ["inss"] = new(AmountLabel: "Salário de contribuição"),
        ["irrf"] = new(AmountLabel: "Base de cálculo (após INSS)", ShowDependents: true, ShowIrrfGrossSalaryToggle: true),
        ["pj-vs-clt"] = new(
            AmountLabel: "Salário bruto CLT",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Faturamento PJ mensal (opcional)",
            ShowDependents: true,
            ShowTransportDiscount: true,
            TransportDiscountLabel: "Descontos CLT (vale-transporte etc.)",
            ShowRate: true,
            RateLabel: "Alíquota Simples Nacional (%)",
            ShowOtherDiscounts: true,
            OtherDiscountsLabel: "Despesas fixas PJ (contador, software etc.)",
            ShowSimplesAnnex: true,
            ShowProLaborePercent: true),
        ["juros-compostos"] = new(
            AmountLabel: "Valor inicial",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Aporte mensal (opcional)",
            ShowMonths: true,
            ShowRate: true,
            RateLabel: "Taxa mensal (%)"),
        ["financiamento"] = new(
            AmountLabel: "Valor financiado",
            ShowMonths: true,
            MonthsLabel: "Prazo (meses)",
            ShowRate: true,
            RateLabel: "Taxa mensal (%)",
            ShowFinancingAmortization: true),
        ["fgts"] = new(
            AmountLabel: "Salário bruto",
            ShowMonths: true,
            MonthsLabel: "Meses trabalhados",
            ShowFgtsBalance: true,
            FgtsBalanceLabel: "Saldo FGTS atual (opcional)",
            ShowFgtsTerminationReason: true),
        ["seguro-desemprego"] = new(
            AmountLabel: "Salário bruto (último mês)",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Salário do 2º mês anterior (opcional)",
            ShowSalaryAverageSupplement: true,
            SalaryAverageSupplementLabel: "Salário do 3º mês anterior (opcional)",
            ShowCompleteYears: true,
            CompleteYearsLabel: "Solicitações anteriores (0 = 1ª, 1 = 2ª, 2 = 3ª ou mais)",
            ShowMonths: true,
            MonthsLabel: "Meses com carteira nos últimos 36 meses",
            ShowThirteenthMonths: true,
            ThirteenthMonthsLabel: "Meses no período de carência exigido",
            ShowTerminationReason: true,
            TerminationReasonLabel: "Motivo do desligamento"),
        ["simulador-mei"] = new(
            AmountLabel: "Faturamento mensal estimado",
            ShowMeiActivity: true,
            ShowMeiAnnualAccumulated: true),
        ["custo-funcionario"] = new(
            AmountLabel: "Salário bruto",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Benefícios mensais (VT, plano etc.)"),
        ["multa-atraso"] = new(
            AmountLabel: "Valor em atraso",
            ShowSecondaryAmount: true,
            SecondaryAmountLabel: "Dias de atraso",
            SecondaryAmountIsDays: true,
            ShowRate: true,
            RateLabel: "Juros ao mês (%)",
            ShowHours: true,
            HoursLabel: "Multa (%)"),
        ["conversor-salario"] = new(
            AmountLabel: "Valor a converter",
            ShowSalaryBasis: true,
            ShowWeeklyWorkHours: true,
            AmountIsCurrency: true)
    };

    public CalculatorFieldProfile GetProfile(string slug)
    {
        return Profiles.TryGetValue(slug, out var profile)
            ? profile
            : new CalculatorFieldProfile();
    }
}
